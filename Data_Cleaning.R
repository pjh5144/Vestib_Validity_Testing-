...
###Cleaning of Medication for Control Purposes###
#Medication Tables
...
SOT<-SOT[,-c(4:13,15:17)]

#History
Hx<-workbook_list$export_Vestib_History
Hx<-Hx[,c(1,3,5,7,9,11)]
Hx[,c(2:6)]<-ifelse(!is.na(Hx[,c(2:6)]),"X","NA")

#Vestib Testing
Testing<-workbook_list$export_Vestib_Combined

Testing<-Testing[,grep("\\_Abnormal|\\ID|\\_Normal",names(Testing))]

Testing$na1<-rowSums(is.na(Testing))
Testing$na2<-rowSums(Testing[]=="NA")
Testing$na3<-rowSums(is.na(Testing)|Testing[]=="NA")

Testing<-Testing%>%
  filter(na3<58) #Review NA's before subsetting. 

Testing<-Testing[,grep("\\_Abnormal|\\ID",names(Testing))]
Testing$Spon_Gaze<-ifelse(Testing$Vestibular_Oculomotor_Spontaneous_Nystagmus_Abnormal=="C"|Testing$Vestibular_Oculomotor_Gaze_Center_Abnormal=="C","C","NA")
...
Testing$Total_C<-rowSums(Testing[,c(3:7,11:34)]=="C",na.rm = TRUE)
